...
TO DO: 
1) include NPT simulation to get box size at reference pressure then run NVT
...
reduced_density = 6000/40.**3 #reduced density of the reference simulation, so that the polymer concentration in the central box is the same with the reference
...
#wall potential and box dimensions
kWall = 1000 *kilocalories_per_mole /angstrom**2
#buffer_fraction = 0.5
wallArea = 40.**2
buffer_l =  #length of pure fluid box, obtained from running NPT at reference pressure with polymer chains enclosed in a central box at reference polymer concentration
#======================
...
volume = (N-N_purefluidbox) * (number_density ** -1) #volume of central box
#box_edge = volume ** (1. / 3.)
central_z = volume/(wallArea*sigma**2)
buffer_l *= sigma
#buffer_l = (N_purefluidbox * (number_density ** -1))/(wallArea*sigma**2)/2
z_wall1 = buffer_l/2
z_wall2 = buffer_l/2 + central_z
box_z = buffer_l + central_z
box_x = (wallArea*sigma**2)**0.5
box_y = box_x
box_vectors = np.diag([box_x/angstrom,box_y/angstrom,box_z/angstrom]) * angstroms
system.setDefaultPeriodicBoxVectors(*box_vectors)
print ('Box vectors:\n{}'.format(box_vectors))
print('Central box length: {}'.format(central_z/angstrom*angstrom))
print('Lenght of buffer region: {}'.format(buffer_l))
print('Number density: {}'.format(N/(box_x*box_y*box_z)*angstrom**3))
#==================
##CREATE TOPOLOGY##
#==================
...
polymer_positions = [box_x/angstrom,box_y/angstrom,central_z/angstrom]*angstrom * np.random.rand(DOP*NP,3)+ [0,0,z_wall1/angstrom]*angstrom

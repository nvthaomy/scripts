...
class FEP:
	...
	def __init__(self,method_,derivative_,traj_list_,states_list_,thermo_files_list_):
		...
		self.ResidueName = str('') # Sets the residue name to be inserted/deleted, uses MDTraj selections
	...
	def SetResidueName(self,ResidueName_):
		...
		self.ResidueName = str(ResidueName_)
	...
	def GenerateMoleculeConfigsInsertions(self,traj,DrawsPerFrame,NumberInsertionsPerFrame):
		...
		if self.ResidueName == '': # no residue set
			...
			res_name = str(self.ResidueName) # the model residue is set
			for resid,residue in enumerate(temp_top.residues):
				if res_name == residue.name:
					temp_resid_list.append(resid)
			model_res = temp_resid_list # indices of residues of type res_name
		...
		self.SetResidueIndiceList_Insertions(model_res) # for record keeping
		...
		for atom in temp_top.residue(model_res[0]).atoms: # add atoms to temp_residue
			t_name 		= atom.name
		...
		MoleculeIndices2Draw	= np.random.randint(0,NumberMolecules,size=(DrawPerFrame*NumberFrames))
		MoleculeIndices2Draw 	= [model_res[i] for i in MoleculeIndices2Draw] 
		...
		NewPositions_List = [] # list of list. Each sublist corresponds to several insertions on one frame
		#Randomly Generate new COM's
		NewCOM = BoxL*np.random.uniform(size=(NumberFrames,NumberInsertionsPerFrame,3))
		...
		return NewPositions_List
		
	def PerformInsertions(self,traj,simulation0,simulation1,NewPositions_List,PotEne_State0_List,ReRunRefState):
		...
		model_res = ''
		if self.ResidueName == '': # no residue set
			model_res = temp_top.residue(0) # The model residue that is to be inserted
		else: 
			res_name = str(self.ResidueName) # the model residue is set
			for resid,residue in enumerate(temp_top.residues):
				if res_name == residue.name:
					model_res = temp_top.residue(resid)
					break
		
		if model_res == '': # check if blank
		...
		NumAtoms = temp_top.n_atoms # Number of atoms after inserting one molecule
		...
		for i in range(NumberFrames):
			...
			xyz[0][0:(-1*int(atoms_added))]=traj[i].xyz[0] # update the initial positions
			...
			for j in range(self.NumberInsertionsPerFrame):
				for ind in range(atoms_added):	
					...
					xyz[0][(-1*t_index)] = NewPositions_List[i][j][ind] # Oxygen
					#xyz[0][-3] = NewPositions_List[i][j][1] # Hydrogen 1
					#xyz[0][-2] = NewPositions_List[i][j][2] # Hydrogen 2
					#xyz[0][-0] = NewPositions_List[i][j][3] # Virtual Site
				
				temp_traj = md.Trajectory(xyz,temp_top)
				
				#temp_traj = md.Trajectory(xyz,traj_System2.topology)
				temp_traj.save_pdb('temp_traj')
	...
	def PerformDeletions(self,traj,simulation0,simulation1,NumberDeletionAttemptsPerFrame,PotEne_State0_List,ReRunRefState):
		...
		if RandomSelect: # Not Recommended
			...
			for j in temp: 
				temp_temp = [model_res[i] for i in j]
		...
		for i in range(NumberFrames):
			...
			for j in range(NumberDeletionAttemptsPerFrame):
				if RandomSelect:
					residueSelect = temp_top.select("resid != {}".format(MoleculeIndices2Delete[i][j]))
				else:
					residueSelect = temp_top.select("resid != {}".format(int(MoleculeIndices2Delete[j])))
		...
		for traj_index, traj in enumerate(self.traj_list):
			...
			if self.derivative == 'particle': # calculating chemical potential 
				...
				if traj_index == 0: # do insertions first 
					...
					NewPositions_List = self.GenerateMoleculeConfigsInsertions(traj,self.NumberDrawsPerFrame,self.NumberInsertionsPerFrame)				
					
					# Do Insertions
					PotEne_Data_0to1_List, PotEne_State0_List = self.PerformInsertions(traj,self.states_list[0],self.states_list[1],NewPositions_List,self.thermo_files_list[0],self.ReRunRefState)

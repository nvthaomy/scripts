...
"""Assumptions:
   ...
   Topology sequence of system1 is n0 Na+, n0 Cl-, Na+, Cl-, nw HOH
	Inserting new NaCl positions before HOH
...
#        self.SetResidueIndiceList_Insertions([x in y for y in model_res]) # for record keeping
    ...
    def PerformInsertions(self,traj,simulation0,simulation1,NewPositions_Lists,PotEne_State0_List,ReRunRefState):
        ...
        for i in range(NumberFrames):
            ...
            xyz[0][0:waterId] = traj[i].xyz[0][0:waterId]
            xyz[0][-1*(traj.n_atoms-waterId):] = traj[i].xyz[0][waterId:]
            ...
            for j in range(self.NumberInsertionsPerFrame):
                for k, atoms_added in enumerate(atoms_added_List):
                    ...
                    if k > 0:
                        positions_added = sum(atoms_added_List[0:k])
                        atoms_added += positions_added
                    elif k==0:
                        positions_added = 0
                        
                    for ind in range(positions_added, atoms_added):    
                        t_index = waterId + ind
                        xyz[0][t_index] = NewPositions_List[i][j][ind - positions_added] # Oxygen

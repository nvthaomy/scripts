...
def HistogramRee(data, number_bins=25, DoBootStrapping=True, ShowFigures=True, NormHistByMax=True, 
	...
	if DoBootStrapping == True:
		[HistAvg, CIPlus, CIMinus, alpha, NormHistByMax, HistAvgValueRee, HistAvgValueStdDevRee] = DoBootStrappingOnHistogram(np.divide(np.asarray(Ree_list),scale),hist,number_Ree_hist_bins,Ree_min,Ree_max)

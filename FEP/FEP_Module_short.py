...
import sys
"""Modified to quickly get chemical potential, dont' need to supply traj and systems to initiate FEP class
"""
...
class FEP:
    # constructor
    def __init__(self,method_,derivative_):
        ''' 
...
#        self.SetResidueIndiceList_Insertions([x in y for y in model_res]) # for record keeping
    ...
    def PerformDeletions(self,traj,simulation0,simulation1,NumberDeletionAttemptsPerFrame,PotEne_State0_List,ReRunRefState):
        ...
        if NumberMoleculesList[0] < 10:
            print('WARNING: There are only {} molecules to select from for deletions. Is this correct?'.format(int(NumberMoleculesList[0])))
        ...
        elif NumberDeletionAttemptsPerFrame > NumberMoleculesList[0]:
            NumberDeletionAttemptsPerFrame = NumberMoleculesList[0]
            print('WARNING: NumberDeletionAttemptsPerFrame > NumberMolecules ; setting to NumberMolecules {}.'.format(NumberMoleculesList[0]))
